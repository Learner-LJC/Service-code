...
public class TeleporterObject2 : MonoBehaviour {
        ...
        if (playControl != null && playControl.isActiveAndEnabled)
        {
            ...
            if (td == null)
            {
                Debug.LogErrorFormat(
                    "Teleporter Object:Character[{0}]Enter Teleporter[{1}] But Teleporter is not existed",
                    playControl.character.Info.Name, this.ID);
                return;
            }

            Debug.LogFormat("Teleporter Object:Character[{0}] Enter Teleporter[{1}][{2}] ",
                playControl.character.Info.Id, td.ID, td.Name);
            if (td.LinkTo > 0)
            {
