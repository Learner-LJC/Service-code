...
namespace GameServer.Services
{
    class MapService : Singleton<MapService>
    {
        ...
        private void OnMapEntitySync(NetConnection<NetSession> sender, MapEntitySyncRequest request)
        {
            ...
            //Log.InfoFormat("OnMapEntitySync: characterID{0} :{1}Entity.Id:{2} Evt{3} Entity{4}",character.Id,character.Info.Name,request.entitySync.Entity.String());
        ...
        {
            Character character = sender.Session.Character;
            Log.InfoFormat("OnMapTeleporter:CharacterID:{0}:{1}TeleportID:{2}",character.Id,character.Data,request.teleporterId);
            if (!DataManager.Instance.Teleporters.ContainsKey(request.teleporterId))
            {
                Log.WarningFormat("Source TeleporterID[{0}] is not existed",request.teleporterId);
                return;
            }
            ...
            {
                Log.WarningFormat("Source TeleporterID[{0}] LinkToID[{1}]",request.teleporterId,source.LinkTo);
